...
class Edge:
    ...
    def __eq__(self, other):
        return (self.beg == other.beg and self.fin == other.fin) or \
            (self.beg == other.fin and self.fin == other.beg)
...
class Polyedr:
    ...
    def count_shadows(self):
        ...
        edges = []
        for e in self.edges:
            cnt = sum([gap.fin - gap.beg for gap in e.gaps])
            if not e.gaps or cnt < 1e-10:
                continue
            if 1 - cnt < 1e-10:
                continue
            if ((e.beg + e.fin) * 0.5).squared_sum() > 4 * self.c * self.c:
                continue
            if e in edges:
                continue
            edges.append(e)
